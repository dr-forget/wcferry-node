...
export class Wcferry {
  ...
  execDLL(verb: "start" | "stop") {
    ...
    // 初始化
    if (verb == 'start') {
      const initResult = this.WxInitSDK(false, this.options.port);

      if (initResult == 0) {
        console.log("wcf====>success");
        this.wcf_isRun = true;

        // 阻塞进程 1s 防止rpc断掉
        this.timerKey = setInterval(() => { }, 1000);
      } else {
  ...
  async downloadImage(
    ...
    if (
      this.downloadAttach(msgid, msgAttachments.thumb, msgAttachments.extra) !==
      0
    ) {
