...
export class FileRef implements FileSavableInterface {
   ...
   * @returns
   */
  async save(dir = os.tmpdir(), cpLocal = false): Promise<{ path: string; discard: () => Promise<void> }> {
    ensureDirSync(dir);
    ...
    return await new Promise<string>((resolve, reject) => {
      http
        .get(url, { headers }, (response) => {
          const probeName = response.headers['content-disposition']?.match(/attachment; filename="?(.+[^"])"?$/i)?.[1] ?? basename;
